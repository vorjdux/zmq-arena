...
#   make build      # orchestrator + the runnable targets (libzmq, monocoque)
...
.PHONY: all build orchestrator libzmq monocoque targets-all \
        bench render run run-root dry dashboard clean help

all: build run            ## build everything, then run + render

build: orchestrator libzmq monocoque  ## build the control plane and the runnable targets
