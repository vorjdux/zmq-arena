//! zmq-arena target wrapper: zeromq-rs (epoll + Tokio work-stealing).
//!
...
//! CLI contract, knob parsing, and role dispatch are complete.
...
#[derive(Parser, Debug)]
#[command(name = "zeromq-rs-target", version, about = "zmq-arena zeromq-rs wrapper")]
struct Cli {
    #[arg(long, value_enum)]
    role: Role,
    #[arg(long, value_enum)]
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "zeromq-rs-target: role={:?} pattern={:?} transport={:?} endpoint={} payload={}B msgs={} warmup={} knobs={:?}",
...
/// TODO(maintainer): open the zeromq-rs PUSH/PUB socket, apply known knobs
/// (`sndhwm`, `rcvhwm`, `io_threads`), send `warmup` then `messages` payloads.
fn run_publisher(_cli: &Cli, _knobs: &BTreeMap<String, String>) -> Result<()> {
    bail!("zeromq-rs publisher loop not implemented");
}

/// TODO(maintainer): open the zeromq-rs PULL/SUB socket, drain `warmup` then
/// receive exactly `messages` payloads without dropping.
fn run_subscriber(_cli: &Cli, _knobs: &BTreeMap<String, String>) -> Result<()> {
    bail!("zeromq-rs subscriber loop not implemented");
}
